...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--risk-level", type=str, default=None, help="预测清单风险级别过滤，逗号分隔 LOW,MEDIUM,HIGH；默认读 job 策略配置")
    ...
    def handle(self, *_args, **options):
        ...
        valuation_variant = str(job_config.get("valuation_variant") or "").strip()
        risk_variant_policy = str(job_config.get("risk_variant_policy") or "any").strip().lower()
        if risk_variant_policy not in {"any", "specific"}:
            risk_variant_policy = "any"
        min_netprofit_yoy = _to_float_or_none(job_config.get("min_netprofit_yoy"))
        min_ebit_yoy = _to_float_or_none(job_config.get("min_ebit_yoy"))
        require_positive_prev_netprofit = self._to_bool(job_config.get("require_positive_prev_netprofit"), True)
        require_positive_prev_ebit = self._to_bool(job_config.get("require_positive_prev_ebit"), True)
        financial_filter_mode = str(job_config.get("financial_filter_mode") or "all").strip().lower()
        if financial_filter_mode not in {"all", "any"}:
            financial_filter_mode = "all"
        priority_policy = str(job_config.get("priority_policy") or "score_desc").strip().lower()
        ...
        financial_panel_map = _load_financial_panel_map(ts_codes, trade_date) if financial_filters_enabled else {}
        ...
        for row in trading_rows:
            ...
            if traditional_min_signal_score is not None and (traditional_score is None or traditional_score < traditional_min_signal_score):
                continue
            risk_payload = traditional_risk_map.get((trade_date, ts_code)) or {}
            if risk_variant_policy == "specific":
                matched_risk_level = str(risk_payload.get("selected_variant_risk_level") or "").strip().upper()
                passes_risk = matched_risk_level in traditional_risk_levels
            else:
                risk_levels = [str(level or "").strip().upper() for level in (risk_payload.get("risk_levels") or [])]
                passes_risk = any(level in risk_levels for level in traditional_risk_levels) if traditional_risk_levels else True
            if not passes_risk:
                continue
            ...
            profit_data_source = str(anchor_payload.get("profit_data_source") or "").strip()
            discount_pct = ((float(conservative_valuation) / float(close_price)) - 1.0) * 100.0 if float(close_price) > 0 else 0.0
            target_discount_pct = ((float(composite_valuation) / float(close_price)) - 1.0) * 100.0 if float(close_price) > 0 else 0.0
            risk_score = risk_payload.get("min_risk_score") if isinstance(risk_payload, dict) else None
            base_row = {
                "tscode": ts_code,
                "stock_name": corp_map.get(ts_code, ""),
                "close_price": round(close_price, 4),
                ...
                "report_end_date": report_end_date_text,
                "trade_date": trade_date_text,
                "_rank_key": _candidate_buy_rank_key(
                    {
                        "score": traditional_score if traditional_score is not None else 0.0,
        ...
        if not traditional_rows:
            self.stdout.write("[warn] No traditional undervalued candidates matched filters; empty-header CSV will still be written.")
        predictive_rows = []
        ...
        traditional_rows.sort(key=lambda item: item.get("_rank_key") or (item.get("tscode") or ""))
        for item in traditional_rows:
