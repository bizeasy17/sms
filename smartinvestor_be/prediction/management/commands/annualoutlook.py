...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--codes-file",
            type=str,
            help="Optional whitelist file with one ts_code per line; applied after scope filter.",
        )
    ...
    def handle(self, *_args, **options):
        ...
        if codes_file:
            ...
            if not codes_path.exists():
                raise CommandError(f"codes file not found: {codes_file}")
            whitelist = {
                line.strip().upper()
            ...
            if not whitelist:
                raise CommandError("codes file is empty")
            trading_qs = trading_qs.filter(ts_code__in=whitelist)
