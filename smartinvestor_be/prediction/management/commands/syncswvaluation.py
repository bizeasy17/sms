...
class Command(BaseCommand):
    ...
    def _safe_write(self, message):
        try:
            ...
            # Scheduled tasks can inherit a non-UTF-8 stdout; fall back to an ASCII-safe rendering.
            fallback = str(message).encode("ascii", errors="backslashreplace").decode("ascii")
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help="参数模板输出文件后缀，例如 ref_5_10_20",
        )
    ...
    def handle(self, *_args, **options):
        ...
        if include_params:
            self._safe_write("提示：参数生成阶段会调用外部接口，期间可能数分钟无输出，属正常现象。")
        ...
        try:
            result = service.sync(
                ...
                params_output_suffix=options.get("params_output_suffix") or "",
            )
        except Exception as exc:
            raise CommandError(str(exc)) from exc

        self._safe_write(self.style.SUCCESS("申万估值同步完成"))
        self._safe_write(f"dry_run: {result['dry_run']}")
        if include_mapping:
            self._safe_write(f"mapping_file: {result['mapping_file']}")
            self._safe_write(f"mapping_levels: {result['mapping_levels']}")
            self._safe_write(f"mapped_ts_codes: {result['mapped_ts_codes']}")
        if include_params:
            self._safe_write(f"params_file: {result['params_file']}")
            self._safe_write(f"params_levels: {result['params_levels']}")
            self._safe_write(f"trade_date: {result['trade_date']}")