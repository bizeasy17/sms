...
class PrefillValuationSnapshotTraceTests(TestCase):
	def test_trace_filters_announcement_before_report_end(self):
		snapshot = {
			...
			"raw_frames": {
				"income": pd.DataFrame(
					[
						{"end_date": "20251231", "ann_date": "20150203"},
		...
		self.assertEqual(trace.get("profit_report_ann_date"), date(2026, 3, 20))

	def test_trace_filters_announcement_after_trade_date(self):
		snapshot = {
			"trade_date": "2026-03-27",
			"end_date": "20251231",
			"profit_data_source": "fina_indicator_income",
			"raw_frames": {
				"income": pd.DataFrame(
					[
						{"end_date": "20251231", "ann_date": "20260402"},
						{"end_date": "20251231", "ann_date": "20260320"},
					]
				),
				"fina_indicator": pd.DataFrame(),
