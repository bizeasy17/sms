...
class ShenwanValuationSyncService:
    def __init__(
        ...
        if market != "CN":
            raise ValueError("申万行业估值同步目前仅支持 CN 市场。")

        self.base_dir = base_dir
        self.market = market
        self.src = src
        self.pro = pro or get_tushare_pro()
        self.mapping_path = base_dir / "valuation_config" / f"sw_industry_mapping_{market}.json"
        self.defaults_path = base_dir / "valuation_config" / f"valuation_defaults_{market}_sw.json"
        self._fina_cache = {}
    ...
    def _build_defaults_path(self, params_output_suffix: str = ""):
        suffix = str(params_output_suffix or "").strip()
        if not suffix:
            return self.base_dir / "valuation_config" / f"valuation_defaults_{self.market}_sw.json"
        return self.base_dir / "valuation_config" / f"valuation_defaults_{self.market}_sw_{suffix}.json"
    ...
    def sync(
        ...
        params_output_suffix: str = "",
    ):
        if not include_mapping and not include_params:
            raise ValueError("至少要同步 mapping 或 params 之一。")
        if request_interval is not None:
            self.request_interval = max(float(request_interval), 0.0)

        defaults_path = self._build_defaults_path(params_output_suffix=params_output_suffix)
        ...
        if include_params:
            ...
            if not dry_run:
                self._write_json(defaults_path, params_payload)

        return {
            "mapping_file": str(self.mapping_path),
            "params_file": str(defaults_path),
            "mapping_levels": (
    ...
    def build_sw_mapping(self):
        ...
        for level in ["L1", "L2", "L3"]:
            ...
            if df is None or df.empty:
                raise ValueError(f"无法获取申万 {level} 分类数据。")
            df = df.fillna("")
    ...
    def _fetch_index_member_all_full(self):
        """
        拉取 index_member_all 全量数据。

        - 优先使用 limit/offset 分页，避免默认返回行数上限导致数据截断。
        - 若当前 tushare 版本不支持分页参数，则自动回退到单次调用。
        """
        ...
        for _ in range(300):
            try:
                ...
                # 当前 SDK 不支持 limit/offset，回退单次调用
    ...
    def build_sw_valuation_defaults(
        ...
        cfg = ValuationConfig(self.base_dir, market=self.market)
        ...
        if daily_basic_df is None or daily_basic_df.empty:
            raise ValueError(f"无法获取 {trade_date_str} 的 daily_basic 数据。")
        ...
        for idx, (l3_code, member_codes) in enumerate(l3_items, start=1):
            ...
            if (
                ...
                and (idx % progress_every == 0 or idx == total_l3)
            ):
                progress_callback(
                    {
                        "stage": "params_l3",
        ...
        return {
            ...
            "trade_date": trade_date_str,
            "sample_size": sample_size,
            "global_defaults": cfg.defaults.get("global_defaults", {}),
            "levels": {
                "L1": l1_nodes,
                "L2": l2_nodes,
                "L3": l3_nodes,
    ...
    def _build_aggregate_node(self, meta, child_nodes, cfg: ValuationConfig):
        base_big, base_bucket, base_params = self._resolve_base_params(meta, cfg)
    ...
    def _resolve_base_params(self, meta, cfg: ValuationConfig):
        for name in [
            meta.get("industry_name"),
            meta.get("parent_name"),
            meta.get("grandparent_name"),
        ]:
            if not name:
                continue
            try:
                big, bucket = cfg.resolve_bucket_by_industry_name(name, fuzzy=True)
                params = cfg.defaults.get("industries", {}).get(bucket, cfg.get_global_params())
                return big, bucket, cfg.normalize_test_valuation_kwargs(params)
            except ValueError:
                continue
        return None, "global_defaults", cfg.get_global_params()
    ...
    def _derive_params_from_metrics(self, metrics, base_params):
        ...
        normalized_growth = self._clamp(
            (growth_pct / 100.0) if growth_pct is not None else 0.08,
            -0.02,
            0.25,
        )
        ...
        sensitivity_grid = {
            "discount_rate": [
                round(self._clamp(discount_rate + shift, 0.07, 0.18), 4)
                for shift in [0.005, 0.002, 0.0, -0.002, -0.005]
            ],
            "terminal_growth_rate": [
                round(self._clamp(terminal_growth + shift, 0.001, 0.035), 4)
    ...
    def _blend_with_history_anchor(
        ...
        # Keep blending deterministic and easy to port into standalone valuation service.
    ...
    def _select_sample_codes(self, member_df, sample_size: int):
        if member_df is None or member_df.empty:
            return []
    ...
    def _resolve_trade_date(self, trade_date=None):
        ...
        for offset in range(15):
            ...
            if df is not None and not df.empty:
                return candidate
        raise ValueError("无法自动推断最近可用的申万交易日，请手动传入 --trade-date。")
