...
TS_CODES = ["002236.SZ", "600036.SH", "601398.SH", "600030.SH", "601318.SH"]
...
for ts in TS_CODES:
    ...
    for r in rows:
        ...
        if old.get('buy_candidate') != new.get('buy_candidate'):
            flips.append((d, old.get('buy_candidate'), new.get('buy_candidate'), old.get('undervalue_score'), new.get('undervalue_score')))
    if not flips:
        continue
    print(f"\n{ts} flip_days={len(flips)}")
    for item in flips:
        d, ob, nb, os, ns = item
        print(f"  {d}: old_buy={ob} -> new_buy={nb}, score {os}->{ns}")
