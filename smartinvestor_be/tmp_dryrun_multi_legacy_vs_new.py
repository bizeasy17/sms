...
TS_CODES = [
    "002236.SZ", "600519.SH", "000001.SZ", "300750.SZ", "600036.SH",
    "601398.SH", "300059.SZ", "002594.SZ", "688981.SH", "000725.SZ",
    "600030.SH", "002415.SZ", "601318.SH", "600276.SH", "002475.SZ",
]
...
print(f"range=[{START},{END}] stocks={len(TS_CODES)}")
print("ts_code\tdays\tchanged_days\tbuy_flip_days\tlatest_old\tlatest_new\tlatest_old_buy\tlatest_new_buy")
for ts in TS_CODES:
    rows = list(
    ...
    changed_days = 0
    buy_flip_days = 0
    latest_old_score = None
    latest_new_score = None
    latest_old_buy = None
    latest_new_buy = None
    ...
    for r in rows:
        ...
        if old.get("undervalue_score") != new.get("undervalue_score") or old.get("buy_candidate") != new.get("buy_candidate"):
            changed_days += 1
        if old.get("buy_candidate") != new.get("buy_candidate"):
            buy_flip_days += 1

        latest_old_score = old.get("undervalue_score")
        latest_new_score = new.get("undervalue_score")
        latest_old_buy = old.get("buy_candidate")
        latest_new_buy = new.get("buy_candidate")

    print(f"{ts}\t{len(rows)}\t{changed_days}\t{buy_flip_days}\t{latest_old_score}\t{latest_new_score}\t{latest_old_buy}\t{latest_new_buy}")
