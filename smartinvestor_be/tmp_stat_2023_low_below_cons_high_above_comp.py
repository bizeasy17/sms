...
annual_low_map = {}
annual_high_map = {}
trade_qs = trading_base.values('ts_code', 'trade_date', 'low_qfq', 'low', 'high_qfq', 'high', 'close_qfq', 'close')
...
for ts, cand_list in candidate_by_ts.items():
    ...
    annual_max_high = annual_high_map.get(ts)
    if annual_min_low is None or annual_max_high is None:
        continue
    conservative_ref = min(item['conservative'] for item in cand_list if item.get('conservative') is not None)
    composite_ref = max(item['composite'] for item in cand_list if item.get('composite') is not None)
    if not (annual_min_low < conservative_ref and annual_max_high > composite_ref):
        continue
    ...
    final_rows.append({
        ...
        'annual_max_high': annual_max_high,
        'min_conservative_ref': conservative_ref,
        'max_composite_ref': composite_ref,
        'max_close': max(close_values),
    })

annual_low_high_pass_ts_count = len(final_rows)
final_hit_ts_count = len(final_rows)

out_dir = os.path.join('output', 'local_valuation_checks')
os.makedirs(out_dir, exist_ok=True)
out_csv = os.path.join(out_dir, 'valuation_2023_annual_low_below_cons_high_above_comp.csv')

sorted_rows = sorted(final_rows, key=lambda x: (-x['hit_days'], x['ts_code']))
with open(out_csv, 'w', newline='', encoding='utf-8-sig') as f:
    writer = csv.writer(f)
    writer.writerow(['ts_code', 'hit_days', 'first_hit', 'last_hit', 'annual_min_low', 'annual_max_high', 'min_conservative_ref', 'max_composite_ref'])
    for r in sorted_rows:
        writer.writerow([
            ...
            f"{r['annual_max_high']:.6f}" if r['annual_max_high'] is not None else '',
            f"{r['min_conservative_ref']:.6f}" if r['min_conservative_ref'] is not None else '',
            f"{r['max_composite_ref']:.6f}" if r['max_composite_ref'] is not None else '',
        ])
...
print(f"annual_low_high_pass_ts_count={annual_low_high_pass_ts_count}")
