...
def _build_run_row_from_archive_dict(row: dict):
    ...
    return {
        ...
        "params": (row or {}).get("params_json") or (row or {}).get("params") or {},
    }
...
def list_traditional_backtest_runs(request):
    ...
    db_rows = [_build_run_row_from_model(item) for item in queryset[:limit]]
...
def _build_buyable_universe_rows(payload, max_rows: int = 500):
    result_payload = payload.get("result") if isinstance(payload, dict) else {}
...
def list_traditional_backtest_run_buy_candidates(request, run_id: int):
    ...
    rows = _build_buyable_universe_rows(payload, max_rows=limit)
