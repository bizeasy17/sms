...
为避免估值引擎单文件继续膨胀，`valuation` 目录下已按职责拆分为三层：

1. `valuation/services/valuation_engine.py`

- 仍作为统一门面（保持历史调用路径和函数签名稳定）。
- 负责调度快照、多方法估值、情景/敏感性总流程。

2. `valuation/services/analysis_tools.py`

- 负责估值结果汇总与展示辅助：
   - `summarize_valuation_range`
   - `format_valuation_range_output`
   - `run_valuation_scenarios`
   - `run_sensitivity_analysis`

3. `valuation/services/sw_scarcity_tools.py`

- 负责行业历史锚与稀缺性加成相关能力：
   - sw_history 参数规范化、缓存解析、上下文构建
   - sw_history 组件行构建与变体标识
   - scarcity 输入推导、基准方法选择、overlay 行构建

设计原则：

- 外部入口稳定：调用方继续从 `valuation_engine` 使用既有函数。
- 内部职责收敛：分析展示和行业历史/稀缺性从主引擎剥离。
- 迁移可回滚：即使拆分，依然通过门面函数保持行为一致。
...
## 11.1 为什么现在是 formal + blended 双轨制

从 2026-04 起，估值快照在存储层按 `profit_data_source` 分桶，支持同一只股票、同一交易日、同一方法下并存两条口径：

1. `formal`：仅正式财报口径（不做 express 融合）。
2. `blended`：允许快报融合（通过严格匹配规则后生效）。

你可以把它理解为：

- `formal` 提供更稳定、可回溯的基准口径。
- `blended` 提供披露窗口内更及时的响应口径。

这里有两个容易误解的点：

1. 双轨制是“可并存”，不是“每个键必有两条”。
2. 当 blended 资格不满足时，blended 桶可能回落到 formal 输入，因此结果可能与 formal 接近甚至一致。

前台读取时，系统会先按请求的报告类型收窄候选，再在该报告类型下选择对应口径的最新快照，避免跨报告期串读。
