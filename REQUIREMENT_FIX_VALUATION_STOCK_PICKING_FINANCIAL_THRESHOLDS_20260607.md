...
新增目标（2026-06-08）：在传统与预测选股结果列表中增加“估值快照更新时间”列，便于判断估值数据新鲜度。

新增目标（2026-06-08）：在传统与预测选股结果列表中增加“财报发布日”列，便于核对收益统计与财报时点。

新增目标（2026-06-09）：财务条件开关默认关闭，且财务条件仅在第二阶段对第一轮结果做过滤，不参与第一轮候选筛选。

新增目标（2026-06-09）：财务条件关闭时不展示净利等财务信息，并跳过对应数据抓取以缩短首轮加载时间。
...
6) 新增财务过滤总开关（接口+前端联动）：
   ...
   - 默认值：`0`（默认关闭，优先返回首轮估值结果）。
    - 行为：
      - 开启：在传统/预测两种模式均执行财务二阶段过滤（净利YOY/EBITYOY/上一年净利或EBIT非负），仅作用于第一轮候选结果。
...
10) 预测结果表新增财务4列展示（接口+前端联动）：
   ...
   - 行为：仅在 `apply_financial_filters=1` 时展示并填充；关闭时不展示财务列且不触发财务补数。
11) 新增估值快照更新时间展示列（接口+前端联动）：
   - 展示范围：传统模式与预测模式结果表均显示。
   - 后端：返回每行估值快照更新时间字段（优先使用估值快照 `updated_at`；无值时返回 `null`）。
   - 前端：新增“快照更新时间”列，统一展示格式（如 `YYYY-MM-DD HH:mm:ss`），无值显示 `-`。
   - 行为：仅影响可视化展示，不改变筛选、排序、分页、导出行为。
12) 新增财报发布日展示列（接口+前端联动）：
   - 展示范围：传统模式与预测模式结果表均显示。
   - 字段来源：优先显示 `valuation_profit_report_ann_date`，预测模式可回退 `financial_ann_date`。
   - 前端：新增“财报发布日”列，统一格式 `YYYY-MM-DD`，无值显示 `-`。
   - 行为：仅影响展示，不改变筛选、排序、分页和导出。
...
10) 预测模式结果表仅在 `apply_financial_filters=1` 时显示财务4列；关闭时不展示且不触发财务补数。
11) 传统与预测结果表均可见“快照更新时间”列；字段有值时展示正确，无值显示 `-`，且不影响现有筛选和排序结果。
12) 传统与预测结果表均可见“财报发布日”列；字段有值时展示正确，无值显示 `-`，且不影响现有筛选和排序结果。

13) 当 `apply_financial_filters=0` 时，后台不再抓取用于财务过滤/财务展示的 `INDICATOR` 与 `BALANCESHEET` 数据；前台不展示净利/EBIT等财务列，待开启开关后再展示并按需取值。

14) 财务条件在开启时仅执行第二阶段过滤，不参与第一轮候选筛选。
