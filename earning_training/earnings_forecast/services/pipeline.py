...
class EarningsForecastPipeline:
    ...
    @staticmethod
    def _build_fy_yoy(values: pd.Series, base_abs_min: float, clip_min: float | None, clip_max: float | None) -> pd.Series:
        current = pd.to_numeric(values, errors="coerce")
        base = current.shift(1)
        yoy = current.divide(base.replace(0, np.nan)) - 1.0
        yoy = yoy.where(base.abs() >= float(base_abs_min))
        if clip_min is not None or clip_max is not None:
            yoy = yoy.clip(lower=clip_min, upper=clip_max)
        return yoy
    ...
    def _load_financial_from_db(self) -> dict[str, pd.DataFrame]:
        ...
        engine = create_engine(financial_db_url)
    ...
    def _load_financial_latest_snapshot_for_ts_code(self, ts_code: str) -> pd.DataFrame:
        data = self.config.get("data", {})
        feature = self.config.get("feature", {})
        if not bool(feature.get("use_financial_feature_snapshot", True)):
            return pd.DataFrame()
        ...
        cached = self._financial_latest_snapshot_cache.get(code)
        if cached is not None:
            return cached.copy()
        ...
        if frame is None or frame.empty:
            return pd.DataFrame()
        self._financial_latest_snapshot_cache[code] = frame.copy()
        return frame
    ...
    def train(self, rebuild_dataset: bool = False) -> dict[str, Any]:
        ...
        sample_weight_cfg = train_cfg.get("sample_weight") or {}
        if bool(sample_weight_cfg.get("enabled", False)):
            base_weight = pd.Series(1.0, index=train.index, dtype="float64")
            time_decay_cfg = sample_weight_cfg.get("time_decay") or {}
            if bool(time_decay_cfg.get("enabled", False)):
                half_life_years = max(1e-6, float(time_decay_cfg.get("half_life_years", 4.0)))
                min_weight = float(time_decay_cfg.get("min_weight", 0.5))
                max_weight = float(time_decay_cfg.get("max_weight", 1.0))
                if max_weight < min_weight:
                    min_weight, max_weight = max_weight, min_weight

                apply_report_types = {
                    str(x).strip().upper()
                    for x in (time_decay_cfg.get("apply_report_types") or [])
                    if str(x).strip()
                }
                if apply_report_types and str(train_cfg.get("report_type") or "").strip().upper() not in apply_report_types:
                    pass
                else:
                    trade_dates = pd.to_datetime(train.get("trade_date"), errors="coerce")
                    if trade_dates.notna().any():
                        ref_date = trade_dates.max()
                        age_years = ((ref_date - trade_dates).dt.days.clip(lower=0) / 365.25).fillna(0.0)
                        decay = np.exp(-np.log(2.0) * age_years / half_life_years)
                        decay = pd.Series(decay, index=train.index, dtype="float64")
                        scaled = min_weight + (max_weight - min_weight) * decay
                        base_weight = scaled.clip(lower=min_weight, upper=max_weight)

            train_sample_weight = base_weight
        ...
        def _fit_with_optional_sample_weight(model: Any, x: pd.DataFrame, y: pd.Series, w: pd.Series | None) -> None:
            if w is None:
                model.fit(x, y)
                return
            w_aligned = pd.to_numeric(w.reindex(x.index), errors="coerce").fillna(1.0).astype("float64")
            try:
                model.fit(x, y, sample_weight=w_aligned.to_numpy())
            except TypeError:
                model.fit(x, y)
        ...
        if reg_mask_train.sum() > 50:
            _fit_with_optional_sample_weight(
                reg,
                x_train.loc[reg_mask_train],
                train.loc[reg_mask_train, reg_target_col],
                None if train_sample_weight is None else train_sample_weight.loc[reg_mask_train],
            )
        ...
        _fit_with_optional_sample_weight(
            clf,
            x_train.loc[cls_mask_train],
            train.loc[cls_mask_train, cls_target_col],
            None if train_sample_weight is None else train_sample_weight.loc[cls_mask_train],
        )
        ...
        if use_industry_models:
            for industry_name, industry_idx in train.groupby(train_industry).groups.items():
                ...
                _fit_with_optional_sample_weight(
                    ...
                    x_i,
                    y_cls_i,
                    None if train_sample_weight is None else train_sample_weight.loc[cls_idx],
                )
                ...
                if len(reg_idx) >= industry_reg_min_rows:
                    ...
                    _fit_with_optional_sample_weight(
                        reg_i,
                        x_train.loc[reg_idx],
                        train.loc[reg_idx, reg_target_col],
                        None if train_sample_weight is None else train_sample_weight.loc[reg_idx],
                    )
        ...
        model_file = str(output_cfg.get("model_file", "models.joblib") or "models.joblib").strip() or "models.joblib"
        metrics_file = str(output_cfg.get("metrics_file", "metrics.json") or "metrics.json").strip() or "metrics.json"

        def _safe_joblib_dump(obj: Any, target_path: Path, retries: int = 3) -> None:
        ...
        _safe_joblib_dump(bundle, self.output_dir / model_file)
        ...
        _safe_joblib_dump(bundle, run_dir / model_file)
        ...
        _safe_joblib_dump(bundle, version_model_path)
    ...
    def predict(
        ...
        live_subset = self._build_live_predict_features(code, requested_report_type=requested_report_type or None)
        ...
        for rt in normalized_types:
            try:
                ...
                if pd.isna(ann_dt):
                    ...
                    age_days = max(0.0, float((pd.Timestamp.now().normalize() - ann_dt.normalize()).days))
    ...
    def _build_live_predict_features(self, ts_code: str, requested_report_type: str | None = None) -> pd.DataFrame:
        code = str(ts_code or "").strip()
        if not code:
            return pd.DataFrame()
        ...
        cached_frame = self._live_feature_cache.get(code)
        if cached_frame is None:
            ...
            try:
                ...
                latest_snapshot = self._load_financial_latest_snapshot_for_ts_code(code)
                ...
                cached_frame = frame
                self._live_feature_cache[code] = frame.copy()
