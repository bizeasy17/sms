...
def _report_type(_endpoint: str, end_date: str) -> str:
    d = _digits8(end_date)
...
class Command(BaseCommand):
    ...
    ENDPOINT_MODELS = {
        ...
        "express_vip": get_financial_endpoint_model("express_vip"),
        "forecast_vip": get_financial_endpoint_model("forecast_vip"),
    }
    ...
    def handle(self, *args, **options):
        ...
        for idx, ts_code in enumerate(ts_codes, start=1):
            endpoint_map = {ep: self._collect_latest_by_period(ts_code, ep) for ep in AUDITED_ENDPOINTS}
            express_map = self._collect_latest_by_period(ts_code, "express_vip")
            forecast_map = self._collect_latest_by_period(ts_code, "forecast_vip")

            keys: set[tuple[str, str]] = set()
            for m in endpoint_map.values():
                keys.update(m.keys())
            keys.update(express_map.keys())
            keys.update(forecast_map.keys())
            ...
            for end_date, report_type in sorted(keys):
                ...
                express_payload, express_updated = express_map.get((end_date, report_type), ({}, None))
                forecast_payload, forecast_updated = forecast_map.get((end_date, report_type), ({}, None))

                ann_date = _pick_ann_date(
                    fina_payload or income_payload or balance_payload or cashflow_payload or express_payload or forecast_payload,
                    "",
                )

                source_updated_at = None
                for dt in [income_updated, fina_updated, balance_updated, cashflow_updated, express_updated, forecast_updated]:
                    if dt is None:
                        continue
                ...
                upserts.append(
                    FinancialFeaturePanel(
                        ...
                        end_date=end_date,
                        ann_date=ann_date,
                        revenue=_to_float(income_payload.get("revenue") or express_payload.get("revenue")),
                        total_revenue=_to_float(income_payload.get("total_revenue") or express_payload.get("revenue")),
                        operate_profit=_to_float(income_payload.get("operate_profit")),
                        total_profit=_to_float(income_payload.get("total_profit")),
                        n_income=_to_float(
                            income_payload.get("n_income")
                            or income_payload.get("n_income_attr_p")
                            or express_payload.get("n_income")
                        ),
                        n_income_attr_p=_to_float(income_payload.get("n_income_attr_p")),
                        basic_eps=_to_float(income_payload.get("basic_eps")),
                        diluted_eps=_to_float(income_payload.get("diluted_eps") or express_payload.get("diluted_eps")),
