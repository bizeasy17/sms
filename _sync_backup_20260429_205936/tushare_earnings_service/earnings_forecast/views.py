...
from .models import EarningsSignalSnapshot, EarningsSignalSnapshotHistory, FinancialIncomeRecord
from .services import EarningsForecastPipeline
...
def predict_latest(request):
    try:
        ...
        pipeline = EarningsForecastPipeline(config_path=config_path)
        if report_type == "FUSION":
            result = pipeline.predict_fusion(ts_code=ts_code)
        else:
            result = pipeline.predict(ts_code=ts_code, requested_report_type=report_type or None)
