...
pushd "%ETL_DIR%"
powershell -NoProfile -ExecutionPolicy Bypass -Command "$cmd = '"%PYTHON_CMD%" manage.py resample --freq=ME --dtype=TRADING --force true'; cmd /c $cmd 2>&1 | Tee-Object -FilePath '%LOG_FILE%' -Append; exit $LASTEXITCODE"
if errorlevel 1 goto :FAILED

echo [STEP] resample funda (ME)
echo [STEP] resample funda (ME) >> "%LOG_FILE%"
powershell -NoProfile -ExecutionPolicy Bypass -Command "$cmd = '"%PYTHON_CMD%" manage.py resample --freq=ME --dtype=FUNDA --force true'; cmd /c $cmd 2>&1 | Tee-Object -FilePath '%LOG_FILE%' -Append; exit $LASTEXITCODE"
if errorlevel 1 goto :FAILED

echo [STEP] fetchcorp
echo [STEP] fetchcorp >> "%LOG_FILE%"
powershell -NoProfile -ExecutionPolicy Bypass -Command "$cmd = '"%PYTHON_CMD%" manage.py fetchcorp'; cmd /c $cmd 2>&1 | Tee-Object -FilePath '%LOG_FILE%' -Append; exit $LASTEXITCODE"
if errorlevel 1 goto :FAILED
popd
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$cmd = '"%PYTHON_CMD%" manage.py monthly_financial_maintenance --scope 60,00,30,68 --latest-only'; cmd /c $cmd 2>&1 | Tee-Object -FilePath '%LOG_FILE%' -Append; exit $LASTEXITCODE"
if errorlevel 1 goto :FAILED
