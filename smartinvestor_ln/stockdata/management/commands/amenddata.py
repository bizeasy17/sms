...
from stockdata.models import Corporation, StockFundamentalHistory
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if ts_code:
                ...
                corp_map = {c.ts_code: c for c in Corporation.objects.filter(ts_code__in=ts_codes)}
                updated_count = 0
                for r in records:
                ...
                print(
                    ...
                    if updated_count else
                    f"No matching corporations or records found to update for trade_date {trade_date}."
                )
                return
            else:
            ...
            for corp in corporations:
                ...
                if start_date_str > end_date_str:
                    print(f"Start date {start_date_str} is after end date {end_date_str} for {corp.ts_code}. Skipping.")
                    continue

                df = pro.daily_basic(ts_code=corp.ts_code, start_date=start_date_str, end_date=end_date_str)
                if df is None or df.empty:
                    print(f"No data returned for {corp.ts_code}.")
                    continue
                records = process_fundamental_records(df.to_dict(orient="records"))
                updated_count = update_field_for_records(corp, records)
                print(f"{corp.ts_code} field '{field}' update complete. Total rows updated: {updated_count}")
