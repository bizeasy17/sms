...
from analysis.models import (
    ...
    StockCostFeature,
    StockFeatures,
    StockFundamentalFeature,
    StockTechFeature,
)
from analysis.utils.stock_util import (
...
def process_and_save_features(
    ts_code, freq, distance, fields, feature_type, features, project_root, plugin=False, plugin_params=None, feature_file_path=None, resume=None
):
    """
    ...
    for corp in corps:
        ...
        if feature_type == "cost":
            stock_feature_qs = StockFeatures.objects.filter(
                ts_code=corp.ts_code,
                freq=freq,
                trade_date__gte="2018-01-02",
            ).values(*fields).order_by("trade_date")
        else:
            stock_feature_qs = StockFeatures.objects.filter(
                ts_code=corp.ts_code,
                freq=freq,
        ...
        def get_yaml_fields(key):
            return read_features_from_yaml(
                f"{project_root}/fields_calc.yaml", feature_type=key
            )
        ...
        plugin_features = []
        if plugin:
            func_name = plugin_params.split("|")[0]
            module = importlib.import_module("analysis.utils.stock_util")
            func = getattr(module, func_name)
            
            func_params_str = plugin_params.split("|")[1]
            func_params = {}
            if func_params_str:
                for param in func_params_str.split(","):
                    if "=" in param:
                        key, value = param.split("=", 1)
                        func_params[key.strip()] = value.strip()
            
            df = func(df, **func_params)
            plugin_features = read_features_from_yaml(feature_file_path, feature_type=func_name)
            
        if feature_type not in ("tech", "fundamental", "cost", "all"):
            raise ValueError(f"Unknown feature_type: {feature_type}")

        if plugin and plugin_features:
            df = df[plugin_features]
        else:
            df = df[features]
        ...
        feature_model_map = {
            "tech": StockTechFeature,
            "fundamental": StockFundamentalFeature,
            "cost": StockCostFeature,
        ...
        try:
            ...
            if plugin and plugin_features:
                # Update or create model instances with feature_record
                # Use bulk_update for better performance if records exist, else bulk_create
                existing_objs = model.objects.filter(
                    corporation=corp,
                    freq=freq,
                    trade_date__in=[r.get("trade_date") for r in feature_record if r.get("trade_date") is not None]
                )
                existing_map = {(obj.trade_date): obj for obj in existing_objs}
                to_update = []
                to_create = []
                for record in feature_record:
                    trade_date = record.get("trade_date")
                    if trade_date in existing_map:
                        obj = existing_map[trade_date]
                        for k, v in record.items():
                            setattr(obj, k, v)
                        to_update.append(obj)
                    else:
                        to_create.append(model(**record, corporation=corp, freq=freq))
                if to_update:
                    model.objects.bulk_update(to_update, fields=[k for k in feature_record[0].keys() if k != "trade_date"])
                if to_create:
            ...
            print(
                f"Saved {len(feature_record)} records to {model.__name__} for ts_code={corp.ts_code}, freq={freq}. timediff={pd.Timestamp.now() - start_timestamp}"
            )
