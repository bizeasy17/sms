...
def is_ma_entangled(
    ...
    for group in ma_groups:
        ...
        ma_values = df[group]
        min_ma = ma_values.min(axis=1).replace(0, np.nan)
        within_threshold = (
            ma_values.sub(min_ma, axis=0).abs().div(min_ma, axis=0) <= threshold
        ).all(axis=1)
        df[colname] = within_threshold.fillna(False).astype(int)
    return df
...
def calc_lower_shadow_shape(
    ...
    df["is_lower_shadow_shape"] = (
        (lower_shadow / total >= shadow_ratio)
        & (body / total <= body_ratio)
        & (upper / total < shadow_ratio)
    ).astype(int)
    return df
