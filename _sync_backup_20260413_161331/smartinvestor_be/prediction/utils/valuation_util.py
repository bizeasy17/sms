...
def _filter_financial_frame_asof(df, trade_date=None):
    ...
    # Never fall back to unfiltered rows here. Returning future-announced rows
    # would leak post-cutoff financials into historical valuation snapshots.
    return filtered
...
def _fetch_tushare_dividend_frame(ts_code, pro=None):
    resolved_pro = pro or get_tushare_pro()
    try:
...
def _fetch_local_financial_frames(ts_code, trade_date=None, pro=None):
    trade_date_obj = None
    if trade_date is not None:
        if isinstance(trade_date, date):
            trade_date_obj = trade_date
        else:
            trade_date_obj = _parse_date_yyyymmdd(trade_date)
        ...
        daily_basic = pd.DataFrame([
            {
                ...
                "close": _safe_float(fundamental_row.get("close"), None),
                # StockFundamentalHistory follows tushare daily_basic units (share in 万股, mv in 万元).
                # Keep raw values here because _build_snapshot_from_frames already converts 万 -> 元/股.
                "total_share": _safe_float(fundamental_row.get("total_share"), None),
                "total_mv": _safe_float(fundamental_row.get("total_mv"), None),
                "circ_mv": _safe_float(fundamental_row.get("circ_mv"), None),
                "pe": _safe_float(fundamental_row.get("pe"), None),
                "pe_ttm": _safe_float(fundamental_row.get("pe_ttm"), None),
                "ps": _safe_float(fundamental_row.get("ps"), None),
                "ps_ttm": _safe_float(fundamental_row.get("ps_ttm"), None),
                "pb": _safe_float(fundamental_row.get("pb"), None),
            }
        ])

    db_alias = str(getattr(settings, "VALUATION_LOCAL_FINANCIAL_DB_ALIAS", "earnings") or "earnings")

    fina_indicator = _query_local_financial_df("""
        SELECT *
        FROM earnings_fin_fina_indicator_vip
        WHERE ts_code = %s
        ORDER BY end_date DESC, ann_date DESC
        LIMIT 8
        """, [ts_code], db_alias=db_alias)
    income = _query_local_financial_df("""
        SELECT *
        ...
        LIMIT 8
        """, [ts_code], db_alias=db_alias)
    balancesheet = _query_local_financial_df("""
        SELECT *
        ...
        LIMIT 8
        """, [ts_code], db_alias=db_alias)
    cashflow = _query_local_financial_df("""
        SELECT *
        ...
        LIMIT 8
        """, [ts_code], db_alias=db_alias)
    dividend = _query_local_financial_df("""
        SELECT *
        ...
        LIMIT 8
        """, [ts_code], db_alias=db_alias)
    express_vip = _query_local_financial_df("""
        SELECT *
    ...
    if enable_remote_dividend_fallback and (dividend is None or dividend.empty):
        dividend = _fetch_tushare_dividend_frame(ts_code=ts_code, pro=pro)
...
def get_stock_valuation_snapshot(
    ...
    if bool(getattr(settings, "VALUATION_USE_LOCAL_FINANCIAL_DB", True)):
        try:
            frames = _fetch_local_financial_frames(ts_code=ts_code, trade_date=trade_date, pro=pro)
...
def _build_sw_history_component_rows(snapshot, sw_history_result):
    ...
    for method in ["pe", "pb", "ps"]:
        ...
        rows.append({"method": method, "equity_value": equity_value, "implied_price": float(implied_price), "applied_multiple": float(target_multiple), "current_multiple": current_multiple, **base_payload})
    return rows
...
def _get_cached_sw_history_payload(base_dir: Path, market: str, industry_code: str):
    ...
    if not any(_safe_float(anchors.get(metric)) for metric in ("pe", "pb", "ps")):
        return None
    return payload
...
def _resolve_sw_history_context(ts_code, trade_date, market="CN", token=None, pro=None, history_years=None, history_quantile=SW_HISTORY_DEFAULT_QUANTILE, history_min_samples=SW_HISTORY_DEFAULT_MIN_SAMPLES):
    ...
    use_remote_fallback = str(
        getattr(settings, "SW_HISTORY_USE_REMOTE_FALLBACK", "1")
    ).lower() in {"1", "true", "yes", "on"}

    if use_local_cache:
        cached_payload = _get_cached_sw_history_payload(base_dir, market, industry_code)
        if isinstance(cached_payload, dict):
            return {"sw_info": sw_info, "history_payload": cached_payload}

    if not use_remote_fallback:
        raise ValueError(
            f"{ts_code} 的 sw_history 本地缓存缺失，且已禁用远端回退。"
        )
...
def _resolve_scarcity_inputs(snapshot, valuations_df, scarcity_kwargs=None):
    ...
    if not bool(enabled):
        return None
    beta = _safe_float(payload.get("beta"), 1.0)
    cap_pct = _safe_float(payload.get("cap_pct"), 80.0)
    ...
    if confidence is None:
        report_date = _parse_date_yyyymmdd(snapshot.get("report_date"))
    ...
    if premium_pct <= 0:
        return None
    return {"beta": beta, "cap_pct": cap_pct, "score": score, "confidence": confidence, "premium_pct": premium_pct}
...
def _select_scarcity_base_row(valuations_df):
    ...
    for _, row in valuations_df.iterrows():
        ...
        if not method or implied_price is None or implied_price <= 0:
            continue
        candidates.append({"method": method, "implied_price": implied_price, "equity_value": equity_value})
    if not candidates:
        return None
...
def _build_scarcity_overlay_row(snapshot, valuations_df, scarcity_kwargs=None):
    ...
    if base_row is None:
        return None
    scarcity_inputs = _resolve_scarcity_inputs(snapshot=snapshot, valuations_df=valuations_df, scarcity_kwargs=scarcity_kwargs)
...
def run_sensitivity_analysis(model_func, base_kwargs, variable_grid):
    ...
    for variable_name, values in variable_grid.items():
        for value in values:
            ...
            records.append({"variable": variable_name, "value": value, "method": valuation.get("method"), "equity_value": valuation.get("equity_value"), "enterprise_value": valuation.get("enterprise_value"), "implied_price": valuation.get("implied_price"), "total_share": valuation.get("total_share")})
...
def estimate_all_supported_methods(ts_code, trade_date=None, token=None, pro=None, pe_target=None, ps_target=None, pb_target=None, peg_target=1.0, ev_ebitda_target=None, dcf_kwargs=None, ddm_kwargs=None, sw_history_kwargs=None, scarcity_kwargs=None, strict_express_match=True, express_max_age_days=180, snapshot=None):
    snapshot = _resolve_snapshot_for_valuation(ts_code=ts_code, trade_date=trade_date, token=token, pro=pro, strict_express_match=strict_express_match, express_max_age_days=express_max_age_days, snapshot=snapshot)
    ...
    if snapshot.get("revenue") is not None:
        results.append(estimate_by_ps(ts_code, target_ps=ps_target or snapshot.get("ps_ttm"), trade_date=trade_date, token=token, pro=pro, strict_express_match=strict_express_match, express_max_age_days=express_max_age_days, snapshot=snapshot))
    if snapshot.get("equity_book_value") is not None:
        results.append(estimate_by_pb(ts_code, target_pb=pb_target or snapshot.get("pb"), trade_date=trade_date, token=token, pro=pro, strict_express_match=strict_express_match, express_max_age_days=express_max_age_days, snapshot=snapshot))
    try:
...
def test_valuation(ts_code, trade_date=None, current_price=None, pro=None, pe_target=None, ps_target=None, pb_target=None, peg_target=1.0, ev_ebitda_target=None, dcf_kwargs=None, ddm_kwargs=None, sw_history_kwargs=None, scarcity_kwargs=None, scenario_model="fcff_dcf", scenario_overrides=None, sensitivity_grid=None, strict_express_match=True, express_max_age_days=180, **_ignored_kwargs):
    snapshot = get_stock_valuation_snapshot(ts_code=ts_code, trade_date=trade_date, pro=pro, strict_express_match=strict_express_match, express_max_age_days=express_max_age_days)
...
def test_valuation_light(ts_code, trade_date=None, pro=None, pe_target=None, ps_target=None, pb_target=None, peg_target=1.0, ev_ebitda_target=None, dcf_kwargs=None, ddm_kwargs=None, sw_history_kwargs=None, scarcity_kwargs=None, strict_express_match=True, express_max_age_days=180, snapshot=None, **_ignored_kwargs):
    snapshot = _resolve_snapshot_for_valuation(ts_code=ts_code, trade_date=trade_date, pro=pro, strict_express_match=strict_express_match, express_max_age_days=express_max_age_days, snapshot=snapshot)
    valuations = estimate_all_supported_methods(ts_code=ts_code, trade_date=trade_date, pro=pro, pe_target=pe_target, ps_target=ps_target, pb_target=pb_target, peg_target=peg_target, ev_ebitda_target=ev_ebitda_target, dcf_kwargs=dcf_kwargs, ddm_kwargs=ddm_kwargs, sw_history_kwargs=sw_history_kwargs, scarcity_kwargs=scarcity_kwargs, strict_express_match=strict_express_match, express_max_age_days=express_max_age_days, snapshot=snapshot)
