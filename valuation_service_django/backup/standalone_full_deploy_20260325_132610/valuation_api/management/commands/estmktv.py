...
class Command(BaseCommand):
    ...
    def _run_single_valuation(
        ...
        runtime_kwargs, scarcity_meta = ScarcityAutoEngine._apply_scarcity_profile(
    ...
    def _handle_business_match_mode(self, ts_code, trade_date, freq, scenario_model, options):
        ...
        baseline_frames = []
        try:
            baseline_runtime_kwargs = self._resolve_params(ts_code=ts_code, market=market, fuzzy=True)
            baseline_source = baseline_runtime_kwargs.get("source_info") or {}
            baseline_result = self._run_single_valuation(
                ts_code,
                trade_date,
                freq,
                scenario_model,
                ...
                persist_context={
                    ...
                    "valuation_variant": self._build_valuation_variant(
                        "sw_l3_baseline",
        ...
        if show_source:
            self.stdout.write(f"business_text_source: {(matched_payload.get('profile') or {}).get('source')}")
            self.stdout.write(
                f"business_match_fallback_profile: {fallback_settings.get('profile_name', 'default')}"
            )
            self.stdout.write(f"business_match_fallback: {fallback_reason}")
        ...
        if (not matches or should_fallback) and not disable_business_fallback:
            ...
            if fallback_match is not None:
                ...
                runtime_kwargs = self._extract_runtime_kwargs(sw_payload)
                runtime_kwargs["source_info"]["source"] = f"{runtime_kwargs['source_info'].get('source')}_business_fallback"
            else:
                runtime_kwargs = self._resolve_params(ts_code=ts_code, market=market, fuzzy=True)
                runtime_kwargs["source_info"]["source"] = f"{runtime_kwargs['source_info'].get('source')}_business_fallback"
            ...
            if show_profit_source:
                self._emit_profit_source(result.get("snapshot") or {})
    ...
    def handle(self, *args, **options):
        ...
        if options.get("est_method"):
            runtime_kwargs["scenario_model"] = str(options.get("est_method")).strip().lower()
        elif options.get("scenario_model"):
            runtime_kwargs["scenario_model"] = str(options.get("scenario_model")).strip().lower()
