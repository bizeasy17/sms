...
def _merge_missing_express_fields(primary_row, fallback_row):
    if not primary_row:
        return fallback_row
    if not fallback_row:
        return primary_row
...
def _build_sw_history_component_rows(snapshot, valuations_df, sw_history_kwargs=None):
    ...
    # UAT baseline: prefer explicit history anchors from assumption metrics.
    assumption = snapshot.get("assumption") or {}
    metrics = assumption.get("assumption_metrics") or {}
    anchors = metrics.get("history_anchors") or {}

    anchor_rows = []
    pe_anchor = _safe_float(anchors.get("pe"))
    if pe_anchor not in (None, 0) and _safe_float(snapshot.get("netprofit")) is not None:
        pe_equity_value = _safe_float(snapshot.get("netprofit")) * pe_anchor
        anchor_rows.append(
            {
                "method": "pe",
    ...
    pb_anchor = _safe_float(anchors.get("pb"))
    if pb_anchor not in (None, 0) and _safe_float(snapshot.get("equity_book_value")) is not None:
        pb_equity_value = _safe_float(snapshot.get("equity_book_value")) * pb_anchor
        anchor_rows.append(
            {
                "method": "pb",
    ...
    ps_anchor = _safe_float(anchors.get("ps"))
    if ps_anchor not in (None, 0) and _safe_float(snapshot.get("revenue")) is not None:
        ps_equity_value = _safe_float(snapshot.get("revenue")) * ps_anchor
        anchor_rows.append(
            {
                "method": "ps",
...
def get_local_valuation_snapshot(
    ...
    if bool(getattr(settings, "ENABLE_TUSHARE_FINANCIAL_FALLBACK", True)):
        # Align with UAT path: if local express row exists but lacks usable growth fields,
        # backfill only missing fields from Tushare express_vip.
        if express_row is None or _resolve_express_growth_pct_local(express_row) is None:
            fallback_express_row = _fetch_tushare_express_row(ts_code=ts_code, trade_date=target_date)
            express_row = _merge_missing_express_fields(express_row, fallback_express_row)
...
def estimate_by_pe_local(snapshot, target_pe=None):
    ...
    applied_pe = _positive(target_pe)
    if applied_pe is None:
        applied_pe = current_pe
    if netprofit is None or applied_pe in (None, 0) or shares in (None, 0):
...
def estimate_by_ps_local(snapshot, target_ps=None):
    ...
    applied_ps = _positive(target_ps)
    if applied_ps is None:
        applied_ps = current_ps
    if revenue is None or applied_ps in (None, 0) or shares in (None, 0):
...
def estimate_by_pb_local(snapshot, target_pb=None):
    ...
    applied_pb = _positive(target_pb)
    if applied_pb is None:
        applied_pb = current_pb
    if equity_book_value is None or applied_pb in (None, 0) or shares in (None, 0):
...
def test_valuation_local_light(
    ts_code,
    trade_date=None,
    current_price=None,
    freq="D",
