...
from market_data.models import MarketBarDailyHistory, Security, StockCostDistributionHistory
from market_data.services.sync import build_sync_plan, execute_sync


class Command(BaseCommand):
    help = 'Check or backfill stock cost history gaps using stock bar coverage as the baseline.'
    ...
    def handle(self, *_args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f'Stock cost backfill completed; total upserted rows: {total_rows}'))
    ...
    def _find_missing(start_date, end_date):
        ...
        cost_counts = dict(
            StockCostDistributionHistory.objects.filter(
                security_id__in=stocks.keys(),
                trade_date__range=(start_date, end_date),
            )
            .values('security_id')
        ...
        missing_codes = sorted(
            (
                ...
                if bar_counts.get(security_id, 0) > cost_counts.get(security_id, 0)
            ),
            key=lambda item: item[1],
        )
        return missing_codes, bar_counts, cost_counts, len(stocks)