...
- 新股上市时间较短、历史交易日少于后端技术指标计算最低要求，或复权行情字段尚未完整时，技术趋势区必须保留固定尺寸并展示局部说明，例如“新股上市时间较短或历史行情不足，后台暂时无法计算技术趋势”。
- 技术趋势接口返回业务数据不足时，前台不得只显示泛化的“加载失败”；应区分“数据不足”“无数据”“权限不足”和“请求失败”，并说明当前状态不影响股票身份、行情和其他研究模块。
- 当价格序列不可用时，动量指标、均线、趋势评分和技术信号可以同时显示“暂无”，不得使用涨跌幅、已有少量行情或前端阈值补算技术结论。
...
- 新股上市时间较短、财务数据尚未形成可用报告，或后台尚未发布估值快照时，不显示空白或默认估值；局部状态应明确说明“新股上市时间较短或财务数据不足，后台尚未发布可用的传统估值结果”。
- 预测估值需要历史行情和财务特征；当预测锚点早于上市日期、指定日期没有行情，或模型特征不足时，局部状态应明确说明“新股上市时间较短或历史行情、财务特征不足，后台尚未生成可用的预测估值结果”。
- 上述数据不足属于业务空状态，可以保留“重试”动作，但不得把它与网络错误、权限不足或服务异常混为同一原因；行情、股票身份和其他研究模块继续正常展示。
