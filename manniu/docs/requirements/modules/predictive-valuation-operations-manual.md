...
当消费到市场级事件时，系统会查询最新预测特征面板，按证券去重后逐只执行预测刷新，最多处理 `--limit` 只股票。因此 `--limit 500` 表示一次市场级事件最多刷新 500 只股票，并不表示无界全市场刷新；超过上限的市场影响范围需要后续批次或提高 limit。普通财务披露、个股状态事件仍按事件本身的证券范围处理。当前 daily batch 使用 `--limit 500`，以限制单次市场级预测刷新规模。
