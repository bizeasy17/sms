...
`--limit 100` 的具体含义分为三层：事件检测阶段最多导入 100 条上游事件；事件消费阶段每轮最多领取 100 条待处理事件；如果消费到市场级风格变化事件，则最多向 100 只股票扇出刷新。普通个股事件通常一条对应一只股票，因此不应将该参数简单理解为始终刷新 100 只股票。超过上限的事件会保留在待处理状态，后续批次继续消费。
