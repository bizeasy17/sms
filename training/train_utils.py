...
params_rf = {
    ...
    # - 欠拟合：增加树的数量（如200或更多）。
    # 树的最大深度
    "max_depth": None,  # 默认值为None（树会完全生长）
    # 调整方向：
    # - 过拟合：限制树的深度（如10、20）。
    # - 欠拟合：增加或保持较大的深度（如None或50）。
    # 每次分裂时考虑的特征数量
    "max_features": "sqrt",  # 默认值为"sqrt"（分类任务推荐）
    # 调整方向：
    # - 过拟合：减少特征数量（如"log2"或一个较小的整数）。
    # - 欠拟合：增加特征数量（如"auto"或总特征数量的一部分）。
    # 叶子节点的最小样本数
    "min_samples_leaf": 1,  # 默认值为1
    # 调整方向：
    # - 过拟合：增加最小样本数（如5、10）。
    # - 欠拟合：减少最小样本数（如1）。
    # 节点分裂所需的最小样本数
    "min_samples_split": 2,  # 默认值为2
    # 调整方向：
    # - 过拟合：增加最小样本数（如5、10）。
    # - 欠拟合：减少最小样本数（如2）。
    # 是否启用样本重采样（Bootstrap）
    "bootstrap": True,  # 默认值为True
    # 调整方向：
    # - 过拟合：保持为True。
    # - 欠拟合：尝试设置为False（使用所有样本）。
    # 每棵树使用的最大样本比例
    "max_samples": None,  # 默认值为None（使用所有样本）
    # 调整方向：
    # - 过拟合：减少样本比例（如0.8或0.6）。
    # - 欠拟合：保持为None或增加比例。
    # 类别权重
    "class_weight": None,  # 默认值为None
    # 调整方向：
    # - 过拟合：保持为None。
    # - 欠拟合（类别不平衡）：设置为"balanced"或"balanced_subsample"。
    # 随机种子
    "random_state": 42,  # 设置随机种子以保证结果可复现
    # 并行处理的线程数
    "n_jobs": -1,  # 默认值为None（使用单线程），-1表示使用所有CPU核心
    # 调整方向：
    # - 过拟合和欠拟合：此参数仅影响训练速度，与模型性能无关。
}
...
params_lgb = {
    ...
    # - 过拟合：增大（如20）。
    # - 欠拟合：减小（如5）。
    "class_weight": {0: 1, 1: 10, 2: 10},  # 类别权重
    "random_state": 42,  # 随机种子
}
