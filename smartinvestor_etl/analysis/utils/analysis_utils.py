...
def identify_stock_top_bottom(ts_code, freq="D", resume=None):
    """
    ...
    for corp in corporations:
        ...
        tops_idx = find_tops(df["high_qfq"])
        bottoms_idx = find_bottoms(df["low_qfq"])
        ...
        StockTopBottomHistory.objects.filter(corporation=corp, freq=freq).delete()
        if records:
...
def calc_top_bottom_gain_loss(
    ts_code, freq="D", entry_type="B", period=None, resume=None
):
    """
    ...
    if period is None:
        try:
            period_env = int(os.getenv("STOCK_ANALYSIS_PERIOD", ""))
        except (TypeError, ValueError):
            period_env = None
        if period_env:
            period = period_env * (2 if freq == "W" else 8 if freq == "M" else 1)
        else:
            period = 14 if freq == "W" else 56 if freq == "M" else 7
            ...
            for top_bottom in top_bottom_histories:
                for time_point in gain_loss_time_points:
                    end_date = top_bottom.trade_date + timedelta(
                        days=period * time_point  # +1是因为不包括start date
                    )
...
def analyze_top_bottom_gain_loss_statistics(
    ...
    if period is None:
        try:
            period_env = settings.STOCK_ANALYSIS_PERIOD
        except (TypeError, ValueError):
            period_env = None
        if period_env:
            period = period_env * (2 if freq == "W" else 8 if freq == "M" else 1)
    ...
    for corp in corps:
        ...
        topbottoms = StockTopBottomHistory.objects.filter(
            corporation=corp, freq=freq, period=period
        ).order_by("trade_date")
