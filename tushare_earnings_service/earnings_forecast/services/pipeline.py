...
class EarningsForecastPipeline:
    ...
    def _safe_sql_ident(name: str) -> str:
        ...
        if not _SQL_IDENT_RE.match(ident):
    ...
    def _build_quantitative_target(
        ...
        # Preserve pre-market-adjustment values so callers can display raw targets.
        implied_return_raw = float(implied_return)
        implied_low_raw = float(implied_low)
        implied_high_raw = float(implied_high)
        ...
        target_price_low_raw = None
        target_price_high_raw = None
        if current_price is not None and current_price > 0:
            target_price_raw = current_price * (1.0 + implied_return_raw)
            target_price_low_raw = current_price * (1.0 + implied_low_raw)
            target_price_high_raw = current_price * (1.0 + implied_high_raw)
        ...
        target_market_cap_raw = None
        target_market_cap_low_raw = None
        target_market_cap_high_raw = None
        if current_market_cap is not None and current_market_cap > 0:
            target_market_cap_raw = current_market_cap * (1.0 + implied_return_raw)
            target_market_cap_low_raw = current_market_cap * (1.0 + implied_low_raw)
            target_market_cap_high_raw = current_market_cap * (1.0 + implied_high_raw)
        ...
        return {
            ...
            "target_return_pct_raw": round(100.0 * implied_return_raw, 4),
            "target_return_low_pct_raw": round(100.0 * implied_low_raw, 4),
            "target_return_high_pct_raw": round(100.0 * implied_high_raw, 4),
            ...
            "target_price_raw": None if target_price_raw is None else round(float(target_price_raw), 4),
            "target_price_low_raw": None if target_price_low_raw is None else round(float(target_price_low_raw), 4),
            "target_price_high_raw": None if target_price_high_raw is None else round(float(target_price_high_raw), 4),
            ...
            "target_market_cap_raw": None if target_market_cap_raw is None else round(float(target_market_cap_raw), 4),
            "target_market_cap_low_raw": None if target_market_cap_low_raw is None else round(float(target_market_cap_low_raw), 4),
            "target_market_cap_high_raw": None if target_market_cap_high_raw is None else round(float(target_market_cap_high_raw), 4),
            "components": {
    ...
    def _load_index_dailybasic_from_local(
        self,
        ts_code: str,
        *,
        start_date: str,
        ...
        if not db_url:
            return pd.DataFrame(), "local_no_db_url"

        table_name = str(data_cfg.get("index_dailybasic_table") or "earnings_mkt_index_dailybasic").strip()
        if not table_name:
            return pd.DataFrame(), "local_no_table"

        try:
            table_sql = self._safe_sql_ident(table_name)
            engine = self._get_engine(db_url)
        except Exception:
            return pd.DataFrame(), "local_engine_error"
        ...
        try:
            ...
            if pd.isna(start_text) or pd.isna(end_text):
                return pd.DataFrame(), "local_bad_date"
            start_iso = start_text.strftime("%Y-%m-%d")
            end_iso = end_text.strftime("%Y-%m-%d")
            sql = text(
                f"SELECT trade_date, pe_ttm, pb, turnover_rate_f "
                f"FROM {table_sql} "
            ...
            return pd.DataFrame(), "local_query_error"

        if frame is None or frame.empty:
            return pd.DataFrame(), "local_empty"

        required_cols = {"trade_date", "pe_ttm", "pb", "turnover_rate_f"}
        if not required_cols.issubset(set(frame.columns)):
            return pd.DataFrame(), "local_bad_schema"
        ...
        if out.empty:
            return pd.DataFrame(), "local_empty"
        return out, "local_index_dailybasic"
    ...
    def _compute_market_overall_adjustment(self, asof_trade_date: Any | None = None) -> dict[str, Any]:
        ...
        prefer_local_source = bool(cfg.get("prefer_local_source", True))
        ...
        for ts_code, idx_weight in index_weights.items():
            ...
            frame = pd.DataFrame()
            source = ""
            if prefer_local_source:
                frame, source = self._load_index_dailybasic_from_local(
                    ts_code=ts_code,
                    start_date=start_date,
                    end_date=end_date,
                )
            ...
            if idx_score is None:
                continue
            if source:
                source_hits.add(str(source))
            weighted_score += float(idx_weight) * float(idx_score)
            weight_sum += float(idx_weight)
            used_index_count += 1

        if source_hits:
            if "local_index_dailybasic" in source_hits and "tushare_index_dailybasic" in source_hits:
                source_used = "mixed_local_tushare"
            elif "local_index_dailybasic" in source_hits:
                source_used = "local_index_dailybasic"
            elif "tushare_index_dailybasic" in source_hits:
                source_used = "tushare_index_dailybasic"
            else:
                source_used = ",".join(sorted(source_hits))
        else:
            source_used = "no_data"
